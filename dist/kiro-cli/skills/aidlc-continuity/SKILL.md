...
- 每台设备使用独立、不可共享的本机 Ed25519 credential 和本机 enrollment；新设备按随机确认语跨回合加入后，仍必须使用自己的 actor/device/client identity 取得、恢复或转移 execution lease。
...
- 代填 team enrollment 确认语、在展示短语的同一回合提交、复制其他设备 private key 或要求团队共享 `AIDLC_TRUST_SECRET`；
...
- `ENROLLMENT_CONFIRMATION_REQUIRED`：已展示随机短语并结束回合，等待用户下一条精确消息；

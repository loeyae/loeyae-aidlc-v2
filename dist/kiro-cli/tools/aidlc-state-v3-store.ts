...
function readStateUnderLock(projectRoot: string, enrollment: EnrollmentRecord | null): WorkflowStateV3 | null {
  ...
  const state = validateWorkflowStateV3(parsed, true);
  if (!enrollment) throw new Error("signed state v3 is not bound to a project enrollment");
...
export function loadWorkflowStateV3(projectRoot: string): WorkflowStateV3 | null {
  ...
  const state = readStateUnderLock(projectRoot, enrollment);
  if (state && enrollment?.status === "pending") registerEnrollment(projectRoot, state.workflow_id);
  return state;
}
...
export function initializeWorkflowStateV3(projectRoot: string, stateValue: WorkflowStateV3): WorkflowStateV3 {
  ...
  return withStateLock(projectRoot, () => {
    ...
    if (!enrollment) registerPendingEnrollment(projectRoot, state.workflow_id);
    ...
    registerEnrollment(projectRoot, state.workflow_id);
    return persisted;
  });
}

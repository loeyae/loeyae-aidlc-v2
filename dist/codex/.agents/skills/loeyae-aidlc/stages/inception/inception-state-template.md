...
- `UI 设计` 区块在签名 I9 choice 写入 history 后按结果派生填充。`UI 设计方式` 只能展示 `html-mock`、`figma`、`跳过` 三值之一；`Figma 来源` 由 `figma-create` / `figma-existing` 分别派生为 `流程创建` / `外部提供`。`页面计划` 指向 canonical `page-plan.md`，HTML/Figma 机器事实分别来自 `ui-mock-manifest.json` / `figma-manifest.json`；Construction 必须读取签名 choice 和这些 canonical 产物，不能用本区块选择模式或补齐字段。
...
- `aidlc-state.json` 是阶段、模块、单元、choice、跳过和审批恢复的唯一机器状态源；handoff.md 只保存由签名状态、canonical 产物和审计派生的人类摘要。冲突时必须阻断并以编排器验证结果为准，禁止由 handoff 反向覆盖。

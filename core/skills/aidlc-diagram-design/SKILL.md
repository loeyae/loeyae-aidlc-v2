...
对于 `flowchart`、`pipeline`、`state` 等过程图，生成 `.diagram.json` 时必须同步生成 `designNotes.layout.mainFlow` 和 `designNotes.layout.loopLanes`。主流程必须覆盖所有业务节点/流程边并可从入口追踪到出口；失败、重试、反馈边必须进入声明的左/右独立 lane 并带可见标签；判定节点必须是 diamond，且每个出口必须有可见分支标签。连接器必须保留 `from/to/port` DOM 属性和完整路径，箭头尖端使用独立 `data-edge-arrow` overlay。

...
9. 仅有源而未执行目标 Provider 时，必须在验收证据中记录目标几何/视觉状态为 `UNVERIFIED`；不得将“目标 Provider / 浏览器视觉验证状态：UNVERIFIED”或等价状态注解写入 SVG 的可见内容、图例、`annotations[]`、`<title>` 或 `<desc>`。用户要求 Provider 但无可验证能力时返回 `NEEDS_CAPABILITY`，不得把源不存在与能力缺失混为一谈。
